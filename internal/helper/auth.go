...
func (a Auth) CreateHashPassword(p string) (string, error) {
	...
	// check: Should lenght of password have minimum 6 character
	if len(p) < 6 {
		return "", errors.New("password should have minimum 6 character")
	}
...
func (a Auth) VerifyToken(t string) (domain.User, error) {
	...
	if claims, ok := token.Claims.(jwt.MapClaims); ok && token.Valid {
		...
		user := domain.User{}
		user.ID = uint(claims["sub"].(float64)) // fukkk
...
func (a Auth) Authorization(ctx *fiber.Ctx) error {
	...
	if authHeader == "" {
		return ctx.Status(401).JSON(&fiber.Map{
			"message": "authrorization header has require",
		})
	}
...
func (a Auth) AuthorizationSeller(ctx *fiber.Ctx) error {
